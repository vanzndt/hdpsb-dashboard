...
<div class="modal-overlay" id="msgModal">
  <div class="modal-box">
    <div class="modal-head">
      ...
      <button class="modal-close" id="modalCloseBtn">Ã¢Å“â€¢</button>
    </div>
...
async function checkNewRows(){
  ...
  buildPicFilters();
    ...
    if(res.hasNew){
      ...
      else showToast("Ã°Å¸â€™Â¬ "+res.rows.length+" pesan baru","success");
    ...
    try{
      ...
      if(prev !== now){
        ...
        if(tr && row.statusKey !== "done"){
          ...
          let html;
          if(klaimOrang){
            html='<div class="chat-input-wrap"><div class="kunci-info">Ã°Å¸â€â€™ Sedang dikerjakan <strong>'+escHtml(now)+'</strong></div></div>';
            tr.classList.add("terkunci");
          } else if(klaimSaya){
            html=buatInputBalas(row,isAdmin,true);
            tr.classList.remove("terkunci");
          } else {
            html='<div class="chat-input-wrap"><button class="klaim-btn" data-rowindex="'+ri+'">Ã¢Å“â€¹ KERJAKAN</button></div>';
            tr.classList.remove("terkunci");
          }
...
function renderTable(){
  ...
  tbody.innerHTML=rows.map(function(row){
    ...
    else if(klaimOrang){ actionCol='<div class="chat-input-wrap"><div class="kunci-info">Ã°Å¸â€â€™ Sedang dikerjakan <strong>'+escHtml(klaimOleh)+'</strong></div></div>'; }
    else if(klaimSaya){ actionCol=buatInputBalas(row,isAdmin,true); }
    else { actionCol='<div class="chat-input-wrap"><button class="klaim-btn" data-rowindex="'+row._rowIndex+'">Ã¢Å“â€¹ KERJAKAN</button></div>'; }
...
async function ambilTiket(rowIndex){
  ...
  if(klaimBtn){ klaimBtn.disabled=true; klaimBtn.textContent="Ã¢ÂÂ³ Mengambil..."; klaimBtn.style.opacity="0.6"; }
  try{
    ...
    if(data.ok){
      ...
      if(klaimBtn){ klaimBtn.disabled=false; klaimBtn.textContent="Ã¢Å“â€¹ KERJAKAN"; klaimBtn.style.opacity=""; }
